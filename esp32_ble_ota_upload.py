...
The key optimization here is a sliding window: up to OTA_WINDOW_SIZE chunks
...
# ── UUIDs ─────────────────────────────────────────────────────────────────────
OTA_SERVICE_UUID = "424F5441-0001-1000-8000-00805F9B34FB"
OTA_CONTROL_UUID = "424F5441-0002-1000-8000-00805F9B34FB"
OTA_DATA_UUID    = "424F5442-0003-1000-8000-00805F9B34FB"
OTA_STATUS_UUID  = "424F5443-0004-1000-8000-00805F9B34FB"
...
OTA_STATUS_ACK      = 0x01
OTA_STATUS_NACK     = 0x02
OTA_STATUS_PROGRESS = 0x03
OTA_STATUS_DONE     = 0x04
...
OTA_CHUNK_SIZE  = 500    # payload bytes per chunk (≤ OTA_BLE_MAX_CHUNK_SIZE)
OTA_WINDOW_SIZE = 8      # chunks in flight before waiting for an ACK
OTA_MAX_RETRIES = 3      # per-chunk retry count on NACK
OTA_ACK_TIMEOUT_SEC = 45.0   # seconds to wait for any single ACK

# firmware validation constants
OTA_ESP_IMAGE_MAGIC      = 0xE9
OTA_ESP_APP_DESC_MAGIC   = 0xABCD5432
OTA_MIN_FIRMWARE_SIZE    = 4096  # anything smaller is certainly wrong
...
class ESP32BLEOTAUploader:
    def __init__(self, firmware: bytes, chunk_size: int = OTA_CHUNK_SIZE,
                 window_size: int = OTA_WINDOW_SIZE, max_retries: int = OTA_MAX_RETRIES,
                 timeout: float = OTA_ACK_TIMEOUT_SEC, password: str = ""):
    ...
    def validate_firmware(self):
        ...
        """
        if len(self.firmware) < OTA_MIN_FIRMWARE_SIZE:
            raise ValueError(
                f"file too small ({len(self.firmware)} bytes) — not a valid firmware image")
        
        # Byte 0: ESP image magic
        if self.firmware[0] != OTA_ESP_IMAGE_MAGIC:
            raise ValueError(
                f"invalid image magic 0x{self.firmware[0]:02X} (expected 0xE9) — "
        ...
        magic_word, = struct.unpack_from("<I", self.firmware, 32)
        if magic_word != OTA_ESP_APP_DESC_MAGIC:
            raise ValueError(
        ...
        version      = self.firmware[32+16 : 32+48].split(b'\x00', 1)[0].decode('ascii', errors='replace')
        project_name = self.firmware[32+48 : 32+80].split(b'\x00', 1)[0].decode('ascii', errors='replace')
        ...
        idf_ver      = self.firmware[32+112: 32+144].split(b'\x00', 1)[0].decode('ascii', errors='replace')
    ...
    async def upload(self, client: BleakClient, on_progress: function) -> None:
        ...
        await client.start_notify(OTA_STATUS_UUID, self._on_notify)
        ...
        # ── OTA_BEGIN ─────────────────────────────────────────────────────
        begin_pkt = struct.pack("<BIIB", OTA_CTRL_BEGIN, total_size, total_crc, len(pw_bytes)) + pw_bytes
        log.info("→ OTA_BEGIN")
        await client.write_gatt_char(OTA_CONTROL_UUID, begin_pkt, response=True)

        pkt = await self._recv_non_progress()
        if pkt[0] != OTA_STATUS_ACK:
        ...
        def progress_bar(done: int) -> str:
            ...
            on_progress(pct, rate, elapsed, eta)
        ...
        while acked_up_to < total_chunks:
            ...
            while sent_up_to < total_chunks and \
                ...
                await client.write_gatt_char(OTA_DATA_UUID, pkt, response=False)
            ...
            if kind == OTA_STATUS_ACK:
                ...
                # on_progress(acked_up_to)
                progress_bar(acked_up_to)

            elif kind == OTA_STATUS_NACK:
                nack_seq  = struct.unpack_from("<H", resp, 1)[0]
                nack_code = resp[3] if len(resp) > 3 else 0xFF
                desc      = OTA_NACK_CODES.get(nack_code, f"0x{nack_code:02X}")
            ...
            elif kind == OTA_STATUS_ERROR:
                code = resp[1] if len(resp) > 1 else 0xFF
                raise RuntimeError(
                    f"Device error: {OTA_NACK_CODES.get(code, f'0x{code:02X}')}")
        ...
        on_progress(100, rate, elapsed, 0)

        # ── OTA_COMMIT ────────────────────────────────────────────────────
        log.info("→ OTA_COMMIT")
        await client.write_gatt_char(OTA_CONTROL_UUID, bytes([OTA_CTRL_COMMIT]), response=True)

        resp = await self._recv_non_progress()
        if resp[0] != OTA_STATUS_DONE:
    ...
    async def abort(self, client: BleakClient) -> None:
        try:
            await client.write_gatt_char(OTA_CONTROL_UUID, bytes([OTA_CTRL_ABORT]), response=True)
        except Exception:
            pass
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    async with client:
        ...
        svc = client.services.get_service(OTA_SERVICE_UUID)
        if svc is None:
...
def main() -> None:
    ...
    p.add_argument("--chunk-size", "-s", type=int, default=OTA_CHUNK_SIZE,
                   help=f"Payload bytes per chunk (default {OTA_CHUNK_SIZE})")
    p.add_argument("--window", "-w", type=int, default=OTA_WINDOW_SIZE,
                   help=f"Chunks in-flight before waiting for ACK (default {OTA_WINDOW_SIZE})")
    p.add_argument("--retries", "-r", type=int, default=OTA_MAX_RETRIES,
                   help=f"Retries on NACK (default {OTA_MAX_RETRIES})")
    p.add_argument("--timeout", "-t", type=float, default=OTA_ACK_TIMEOUT_SEC,
                   help=f"ACK timeout seconds (default {OTA_ACK_TIMEOUT_SEC})")
